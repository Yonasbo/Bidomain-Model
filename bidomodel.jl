...
begin 
    using PlutoUI,HypertextLiteral, ExtendableGrids, VoronoiFVM, PlutoVista,GridVisualize,LinearAlgebra,DifferentialEquations
...
# ╔═╡ 2f7298fb-9109-4724-b604-ed4db47db7e5
norm(Matrix([ 0.263 0; 0 0.0263])*(vec([1 0])-vec([2 2.4])))

# ╔═╡ e69a2b7d-41c9-4ee4-9edc-54ab68299985

md"""
Dimension $(@bind dimension Radio(["9","4"],default="4"))
"""
...
function diffusion(f,u,edge)
	...
	f[3]=-(σ_i*(u[1,1]-u[1,2])+(σ_i+σ_e)*(u[3,1]-u[3,2]))
end
...
# ╔═╡ 1376e0d6-0393-4547-9674-9b3bcbdf8b9f
function bc_99(y,u,node)
	boundary_dirichlet!(y,u,node; region=1,species=3,value=0.0)
	boundary_neumann!(y,u,node; species=1,value=0)
	boundary_neumann!(y,u,node; species=3,value=0)
end;

# ╔═╡ 686e116d-063c-4a5d-b204-fe844c0c9040
if dim==1
	diffu=diffusion
else
	diffu=diffusion_2d
...
# ╔═╡ 71f576cc-fbf6-4559-80ff-3e65a776ea4a
...
# ╔═╡ 02b270ea-85fa-48da-b17e-daf67dcb8239
begin
	...
	control.Δu_opt=2
	tsol=VoronoiFVM.solve(system,inival=inival,times=[0,tend],control=control,log=true,damp=0.5,damp_grow=1.8)
end;
...
scalarplot!(vis,grid,sol[1,:],limits=(-2,2),show=true,label="u",linestyle=:line)
